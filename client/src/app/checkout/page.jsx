...
const CheckoutPage = () => {
  ...
  const handleMtnPayment = async () => {
    if (loadingBtn.mtn || isAnyActionLoading) return;
    if (!requireAddress()) return;
    ...
    try {
      ...
      const response = await Axios.post(
        ...
        {
          headers: {
            "X-Idempotency-Key": generateIdempotencyKey("payunit-mtn"),
          },
        }
      );
  ...
  const handleOrangePayment = async () => {
    if (loadingBtn.orange || isAnyActionLoading) return;
    if (!requireAddress()) return;
    ...
    try {
      ...
      const response = await Axios.post(
        ...
        {
          headers: {
            "X-Idempotency-Key": generateIdempotencyKey("payunit-orange"),
          },
        }
      );
