...
const CheckoutPage = () => {
  ...
  // Guest user address stored in local storage
  ...
  const selectedAddress = isAuthenticated ? (addressList[selectAddress] || {}) : guestAddress;

  // Load guest address from local storage
  useEffect(() => {
    if (!isAuthenticated) {
      const savedGuestAddress = typeof window !== 'undefined' ? localStorage.getItem('guestAddress') : null;
  ...
  // Fetch addresses on component mount (for authenticated users)
  useEffect(() => {
  ...
  // Update showAddressForm state based on addressList or guestAddress
  useEffect(() => {
    if (isAddressLoaded) {
      if (isAuthenticated) {
  ...
  useEffect(() => {
    if (isAuthenticated && addressList.length > 0 && isAddressLoaded) {
      const firstValidIndex = addressList.findIndex(addr => addr.status);
      if (firstValidIndex >= 0) {
        setSelectAddress(firstValidIndex);
      }
    }
  }, [addressList, isAddressLoaded, isAuthenticated]);

  // --- Notify if no address on order attempt ---
  const requireAddress = () => {
    if (isAuthenticated) {
      if (!addressList.length) {
        toast.error("Please add an address before proceeding with your order.", { position: 'top-center' });
        setShowAddressForm(true);
        return false;
      }
      if (!selectedAddress?._id) {
        toast.error("Select your address before placing your order.", { position: 'top-center' });
        return false;
      }
    } else {
      if (!guestAddress) {
        toast.error("Please add your delivery information before proceeding.", { position: 'top-center' });
        setShowAddressForm(true);
        return false;
      }
    }
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (responseData.success) {
          ...
          router.push('/success?text=Order');
        }
      } else {
        const response = await Axios({
        ...
        if (responseData.success) {
          ...
          if (typeof window !== 'undefined') {
            ...
            localStorage.setItem('guestOrderId', responseData.orderId);
          }
          if (fetchCartItem) fetchCartItem();
          router.push('/success?text=Order');
        }
  ...
  const onSubmitAddress = async (data) => {
    try {
      if (isAuthenticated) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('guestAddress', JSON.stringify(guestAddressData));
        }
  ...
  // Payment button rendering - always enabled/clickable
  ...
  // Login or register button for guest users
        ...
        <div className="flex items-start">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-blue-700">
              <p>You're checking out as a guest. Want to save your information for next time?</p>
            </div>
      ...
      <div className='container mx-auto px-4 flex flex-col lg:flex-row w-full gap-6 justify-between'>
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className='text-xl font-semibold text-gray-800'>
                {isAuthenticated ? "Your Delivery Address" : "Delivery Information"}
              </h3>
              {isAddressLoaded &&
                ((isAuthenticated && addressList.length > 0) || (!isAuthenticated && guestAddress)) &&
                !showAddressForm && (
                  <button
                ...
                <div className='flex justify-between items-center gap-4 border-b pb-3 mb-4'>
                  <h2 className='font-bold text-lg text-gray-800'>
                    {isAuthenticated
                      ...
                      : (guestAddress ? "Update Delivery Information" : "Enter Delivery Information")}
                  </h2>
                  {((isAuthenticated && addressList.length > 0) || (!isAuthenticated && guestAddress)) && (
            ...
            {isAddressLoaded &&
              ((isAuthenticated && !addressList.length) || (!isAuthenticated && !guestAddress)) &&
              !showAddressForm && (
                <div className="mb-4 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 font-semibold flex items-center rounded-lg">
                  ...
                  <div>
                    <p>No delivery {isAuthenticated ? "addresses" : "information"} found.</p>
                    <button
                      onClick={() => setShowAddressForm(true)}
                      className="text-blue-600 underline text-sm mt-1 font-normal"
                    >
                      Click here to {isAuthenticated ? "add your first address" : "enter your information"}
                    </button>
                  </div>
                </div>
              )}
            ...
            {!isAuthenticated && isAddressLoaded && guestAddress && !showAddressForm && (
              <div className='bg-white p-5 rounded-lg shadow-md'>
                <h4 className="font-medium text-sm text-gray-500 mb-3">Your delivery information</h4>
                <div className='border rounded-lg p-4 bg-blue-50'>
                  <p className="font-bold text-gray-800">{guestAddress.name}</p>
                  <p className="text-sm text-gray-600">{guestAddress.customer_email}</p>
                  <p className="text-sm text-gray-700">{guestAddress.address_line}</p>
                  <p className="text-sm text-gray-700">{guestAddress.city}, {guestAddress.state}</p>
                  <p className="text-sm text-gray-700">{guestAddress.country} - {guestAddress.pincode}</p>
                  <p className="text-sm font-medium mt-1 text-gray-800">{guestAddress.mobile}</p>
                  <button
                    onClick={() => setShowAddressForm(true)}
        ...
        <div className='w-full lg:w-2/5'>
          <div className='bg-white rounded-lg shadow-md p-5 sticky top-4'>
            ...
            {(isAuthenticated ? (!addressList.length || !selectedAddress?._id) : !guestAddress) && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                <p className="text-sm text-yellow-700 flex items-start">
                  ...
                  <span>
                    You need to {isAuthenticated ?
                      (addressList.length ? 'select' : 'add') + ' a delivery address' :
                      'provide your delivery information'} before you can proceed with payment.
                  </span>
                </p>
              </div>
            )}
