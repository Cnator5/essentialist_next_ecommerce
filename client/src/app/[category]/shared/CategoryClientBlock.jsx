...
export default function CategoryClientBlock({ categorySlug, fallbackTitle, fallbackDesc }) {
  ...
  const fetchData = async () => {
    try {
      ...
      // 1) Get all categories and find by slug
      const catResponse = await Axios({ ...SummaryApi.getCategory })
      const catList = catResponse?.data?.data || []

      // Try to match by slug or name
      const matched = catList.find((c) => {
        const categoryUrlSlug = valideURLConvert(c?.name || '').toLowerCase()
        return categoryUrlSlug === categorySlug.toLowerCase()
      })
      ...
      const prodResponse = await Axios({
        ...SummaryApi.getProductByCategory,
        data: { id: matched._id },
      })
      const prodList = Array.isArray(prodResponse?.data?.data) ? prodResponse.data.data : []
      setProducts(prodList)
    } catch (err) {
      console.error('Category fetch error:', err)
      setError('Unable to load category at the moment.')
      AxiosToastError(err)
    } finally {
      setLoading(false)
    }
  ...
  const avgPrice = useMemo(() => {
    const nums = products.map((p) => Number(p.price)).filter((n) => Number.isFinite(n))
    if (!nums.length) return 0
    return Math.round(nums.reduce((a, b) => a + b, 0) / nums.length)
  }, [products])

  // JSON-LD with ItemAvailability per Schema.org
  const jsonLd = useMemo(() => {
    ...
    const list = {
      ...
      itemListElement: products.slice(0, 24).map((product, idx) => {
        ...
        const brandName = typeof product.brand === 'string' ? product.brand : product.brand?.name || undefined
  ...
  }, [title, description, categorySlug, products, category])
  ...
  if (error || !category) {
    return (
      <div className="text-center bg-white/90 backdrop-blur-sm rounded-xl p-8 shadow-lg border border-pink-200">
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{error || 'Category Not Found'}</h2>
      ...
      <header className="text-center mb-8 bg-white/90 backdrop-blur-sm rounded-xl p-8 shadow-lg border border-pink-200">
        ...
        {brands.length > 0 && (
          <p className="text-gray-600">
            Featured Brands: {brands.slice(0, 6).join(', ')}{brands.length > 6 ? '...' : ''}
          </p>
        )}
      </header>
        ...
        <section className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-pink-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-pink-500 to-pink-600 text-white p-6">
            ...
            <p className="text-pink-100">
              All {products.length} {category.name.toLowerCase()} products with current pricing
            </p>
          </div>
