...
export async function generateMetadata({ params }) {
  ...
  const name = product?.name || 'Product'
  const descRaw = product?.description || `Buy ${name} at the best price from EssentialisMakeupStore.`
  const description = stripHtml(descRaw).slice(0, 300)
  const img = Array.isArray(product?.image) ? product.image[0] : product?.image
  const url = `https://www.esmakeupstore.com/${params.category}/${params.subCategory}/${slug}`
...
function StructuredData({ product, slug, rating, params }) {
  const url = `https://www.esmakeupstore.com/${params.category}/${params.subCategory}/${slug}`
  const imgList = Array.isArray(product?.image) ? product.image : [product?.image].filter(Boolean)
  const offers = {
    '@type': 'Offer',
...
export default async function ProductDisplayPage({ params }) {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: tabularStyles }} />
      <StructuredData product={productData} slug={slug} rating={ratingSSR} params={params} />
