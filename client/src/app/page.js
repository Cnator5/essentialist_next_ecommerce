...
// Important: prefer GET endpoints for cacheability.
// If your backend only supports POST, keep POST but add next: { revalidate } and tags.
const baseURL = process.env.NEXT_PUBLIC_API_URL

const SummaryApi = {
  // Ensure your backend supports GET for these. If not, switch back to POST but keep tags/revalidate.
  getCategory: { url: '/api/category/get', method: 'get' },
  getSubCategory: { url: '/api/subcategory/get', method: 'get' }, // changed to GET for caching
}
...
// Small wrapper to ensure consistent fetch options
async function safeFetch(url, init, { revalidate, tags } = {}) {
  const res = await fetch(url, {
    ...init,
    // Opt INTO caching with tags + timed revalidation
...
// Use memoized functions so multiple calls during a render pass don’t duplicate work
async function getCategories() {
  try {
    const data = await safeFetch(`${baseURL}${SummaryApi.getCategory.url}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
...
async function getSubCategories() {
  try {
    const data = await safeFetch(`${baseURL}${SummaryApi.getSubCategory.url}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json' },
    }, { revalidate: 600, tags: ['subcategories'] })
    return Array.isArray(data) ? data : data?.data || []
  } catch {
    return []
  }
...
export async function generateMetadata() {
  ...
  const dynTitle = top ? DEFAULT_TITLE : `Shop ${top} & More - Best Makeup Store in Cameroon`
...
export const revalidate = 600 // enable Full Route Cache with ISR for the home page
