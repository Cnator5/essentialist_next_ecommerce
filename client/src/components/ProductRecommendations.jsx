...
const CompactHistoryCard = ({ product, onClick }) => {
  ...
  return (
    <div
      ...
      onClick={() => onClick(product)}
    >
      <div className="relative h-36 bg-slate-100 rounded-lg flex items-center justify-center p-2 overflow-hidden border border-gray-200">
        <Image
...
const ProductRecommendations = ({ currentProductId, currentProductData }) => {
  ...
  // --- EFFECT 1: Add current product to history and ensure uniqueness ---
  useEffect(() => {
    if (!currentProductId || !currentProductData?._id) return;
    const oldHistory = getHistory();
    const combinedHistory = [currentProductData, ...oldHistory];
    const uniqueHistoryMap = new Map();
    combinedHistory.forEach(product => {
      if (product && product._id && !uniqueHistoryMap.has(product._id)) {
        uniqueHistoryMap.set(product._id, product);
      }
    });
    const updatedHistory = Array.from(uniqueHistoryMap.values()).slice(0, 15);
    setHistory(updatedHistory);
  }, [currentProductId, currentProductData]);
  ...
  }, [isHomePage, currentProductData, currentProductId, pathname]);

  // **Main change: Use SEO-friendly product URL**
  const handleClickProduct = (product) => {
      ...
      <div className="container mx-auto px-4 space-y-12">
        {relatedProducts.length > 0 && (
          <div className="border-t pt-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:flex gap-4 sm:gap-6 md:gap-10 container mx-auto overflow-x-auto scrollbar-none scroll-smooth">
              {relatedProducts.map((product) => (
                <div key={`related-${product._id}`} className="flex-shrink-0 w-full sm:w-auto md:w-52">
                  <CardProduct data={product} onClick={() => handleClickProduct(product)} />
                </div>
        ...
        {displayHistory.length > 0 && (
          <div className="border-t pt-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 md:flex gap-4 sm:gap-6 md:gap-10 container mx-auto overflow-x-auto scrollbar-none scroll-smooth">
              {displayHistory.map((product) => (
                <div key={`history-detail-${product._id}`} className="flex-shrink-0 w-full sm:w-auto md:w-52">
                  <CardProduct data={product} onClick={() => handleClickProduct(product)} />
                </div>
              ))}
            </div>
